...
The UI provides an interface for the user to interact. The interface is primarily a text box where the user can input and submit text to be analyzed or used for searching the corpus of articles. The text is passed on to the handler which uses the other modules for comparison and visualization purposes.
...
**Outputs:**  
* `image` : A png image for the word cloud 
...
The handler component is the engine. It creates and stores the models and data to be used for analysis. It has functions that aggregate the data from the other modules into interface-ready results. These include 1) listing the recommended articles 2) returning the query sentiment data 3) returning the top topics for the query article and 4) creating a word cloud with top words. A sample mockup is below.
...
### New York Times Article Retriever

The New York Times modules uses the NY Times API to pull recent article summaries and labels and uses this data to identify top seed words to use in the guided LDA model. This module includes methods for retrieving recent NY Times data, aggregating all available API data and summarizing the data into a list of lists where each list is the set of seed words for a guided topic.

**Inputs**:
* None  

**Outputs**:
* `Seed Words`: A file containing the list of seeds words to use in the LDA as well as functions for returning the list.
