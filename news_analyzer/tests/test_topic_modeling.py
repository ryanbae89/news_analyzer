...
class TestTopicModeling(unittest.TestCase):
    ...
    def test_topic_modeler(self):
        ...
        with self.assertRaises(Exception) as context:
            test_model = test_modeler.fit(dtm=self.test_dtm, seed_topics=[], seed_confidence=0.5)
        self.assertTrue('Please enter a dictionary for seed_topics.' \
            in str(context.exception))
        with self.assertRaises(Exception) as context:
            test_model = test_modeler.fit(
                dtm=self.test_dtm, seed_topics=self.seed_topics, seed_confidence=1)
        ...
        test_modeler = topic_modeling.TopicModeler(
            n_topics=len(self.seed_topics), n_iter=100, random_state=0, refresh=20)
        test_model = test_modeler.fit(
            dtm=self.test_dtm, seed_topics=self.seed_topics, seed_confidence=0.5)
        self.assertTrue(isinstance(test_model, guidedlda.guidedlda.GuidedLDA))
